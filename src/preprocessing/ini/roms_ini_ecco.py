...
import sys
import os
sys.path.append(os.path.join(os.environ['proj_dir'],'src'))
...
def roms_ini_ecco(grid_file, theta_file, salt_file, output_file, Tcline, theta_s, theta_b, hc, N, nbdry_ecco,tempbounds,saltbounds,vostock):
    ...
    id = Dataset(theta_file, 'r')
    lon_ecco_raw = id.variables['LONGITUDE_T'][:]
    lat_ecco = id.variables['LATITUDE_T'][0:nbdry_ecco]
    depth_ecco_raw = id.variables['DEPTH_T'][:]
    theta_raw = transpose(id.variables['THETA'][0,:,0:nbdry_ecco,:])
    id.close()
    id = Dataset(salt_file, 'r')
    salt_raw = transpose(id.variables['SALT'][0,:,0:nbdry_ecco,:])
    id.close()
